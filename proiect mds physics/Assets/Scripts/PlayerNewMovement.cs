...
public class PlayerNewMovement : MonoBehaviour {
	...
	void FixedUpdate () 
	{
		...
		if (canGetVelocity)
		{
			if(Input.GetKey(KeyCode.W)){
				Player.velocity += transform.forward * Time.deltaTime * movementSpeed;
            ...
            if (Input.GetKey(KeyCode.D)){
				Player.velocity += transform.right * Time.deltaTime * movementSpeed;
				if (!footsteps.isPlaying && isGrounded)
                    SoundManager.PlayCustom(footsteps, 0.05f);
            }
	...
	{
		if (collision.collider.tag == "Wall" && !isGrounded) // wall running mode
